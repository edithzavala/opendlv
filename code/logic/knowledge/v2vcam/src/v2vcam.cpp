...
V2vCam::V2vCam(int32_t const &a_argc, char **a_argv)
    : TimeTriggeredConferenceClientModule(
      a_argc, a_argv, "knowledge-v2vcam")
    , m_sendLog()
    , m_receiveLog()
    , m_timeType2004()
    , m_printInbound()
...
void V2vCam::setUp()
{
  ...
  m_printOutbound = kv.getValue<bool>(
          "knowledge-v2vcam.printOutbound");
  m_printInbound = kv.getValue<bool>(
          "knowledge-v2vcam.printInbound");
  m_record = kv.getValue<bool>(
          "knowledge-v2vcam.record");
  m_debug = kv.getValue<bool>(
          "knowledge-v2vcam.debug");
    ...
    if (m_simulation) {
      ...
      if (m_printOutbound) {
        std::cout << "Cam object -" << " Message Id: "
                ...
                << " Longitude: "
                << std::to_string(GetLongitude()) << std::endl;
      }
...
void V2vCam::ReadVoice(Voice const &a_voice)
{
