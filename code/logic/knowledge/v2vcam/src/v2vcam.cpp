...
V2vCam::V2vCam(const int32_t &a_argc, char **a_argv) :
        TimeTriggeredConferenceClientModule(a_argc, a_argv, "knowledge-v2vcam"), m_sendLog(), m_receiveLog(), m_timeType2004(), m_printInbound(), m_printOutbound(), m_record(), m_debug(
                ...
                0), m_containerMask(0), m_stationType(0), m_latitude(
                90.0000001), m_longitude(180.0000001), m_semiMajorConfidence(
                -1), m_semiMinorConfidence(-1), m_semiMajorOrientation(-1), m_altitude(
                8000.01), m_heading(-1), m_headingConfidence(-1), m_speed(-1), m_speedConfidence(
                -1), m_vehicleLength(-1), m_vehicleWidth(-1), m_longitudinalAcc(
  ...
  while (getModuleStateAndWaitForRemainingTimeInTimeslice()
    ...
    if (m_active) {
      ...
      if (m_simulation) {
        ...
        if (m_printOutbound) {
          std::cout << "Cam object -" << " Message Id: "
                  << std::to_string(GetMessageId()) << std::endl;
        }
...
//      return;
//    }
//    std::cout << "-------------------------------------Read cam message from v"
//            << a_c.getSenderStamp() << std::endl;
//    ReadVoice(voice);
//  }
}
  ...
  if (frameId == 0) {
    ...
    if (m_debug) {
      std::cout << "DynamicState received (frame 0) - speed=" << m_speed
              << " yawRatevalue=" << m_yawRateValue
              << " longitudinalAcceleration=" << m_longitudinalAcc << std::endl;
    }
  }
}
...
//                    + std::to_string(altitude) + +"," + std::to_string(heading)
...
//
//    opendlv::data::environment::WGS84Coordinate ourGeolocation(m_latitude,
//            m_longitude);
...
//
//    if (m_debug) {
//      std::cout << "Snowfox position: (" << xOffset << "," << yOffset
//              << "), distance: " << distance << ", azmiuth: " << azimuth
//              << ", our heading: " << heading1 << std::endl;
//    }
//
...
//    double azimuthCorner = std::atan2(localYOffset - vehicleWidth / 2.0,
//            localXOffset);
//
...
//    uint16_t objectId = -1;
//
//    opendlv::perception::Object detectedObject(now, type, typeConfidence,
//            objectDirection, objectDirectionConfidence, objectDirectionRate,
//            objectDirectionRateConfidence, distance, distanceConfidence,
//            angularSize, angularSizeConfidence, angularSizeRate,
//            angularSizeRateConfidence, confidence, sources, properties,
//            objectId);
