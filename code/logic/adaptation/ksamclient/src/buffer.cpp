...
Buffer::Iterator::Iterator(const Buffer *a_outer_buffer) :
...
std::shared_ptr<std::vector<unsigned char>> Buffer::Iterator::ReadBytes()
{
  ...
  const int start_pos = m_read_pos;
  const int end_pos = start_pos + count - 1;
...
Buffer::Buffer(const std::vector<unsigned char> &a_bytes) :
...
void Buffer::Append(const std::vector<unsigned char> &a_bytes)
{
  if(m_reversed)
  {
...
void Buffer::AppendBytes(const std::vector<unsigned char> &a_bytes)
{
...
void Buffer::AppendBytesRaw(const std::vector<unsigned char> &a_bytes)
{
  Append(a_bytes);
}
...
void Buffer::AppendString(const std::string &a_string)
{
...
void Buffer::AppendStringRaw(const std::string &a_string)
{
  std::vector<unsigned char> append(a_string.begin(), a_string.end());
  Append(append);
}

const std::vector<unsigned char> &Buffer::GetBytes() const
{
  return m_bytes;
}
